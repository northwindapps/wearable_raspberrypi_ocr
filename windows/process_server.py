...
# Piperの音声モデル(.onnx)へのパス（Windowsパスで指定）
PIPER_MODEL = os.environ.get("PIPER_MODEL", r"C:\models\en_US-lessac-medium.onnx")
...
def speak(text: str) -> None:
    # Windowsではファイルを開いたまま他プロセス(piper)が書き込めないため、
    # 一旦クローズしてパスだけ使う（delete=Falseで手動削除）
    ...
    try:
        ...
        winsound.PlaySound(audio_file.name, winsound.SND_FILENAME)
    finally:
